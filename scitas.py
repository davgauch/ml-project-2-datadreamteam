...
def main(rank, world_size, epochs, save_every, data_dir, working_dir, batch_size, bayesian):
    ...
    trainer = Trainer(model, train_dataset, val_dataset, test_dataset, gpu_id=rank, batch_size=batch_size, epochs=epochs, model_snapshot_file=f"{working_dir}/model.pt", train_loss_file=f"{working_dir}/train_losses.csv", test_loss_file=f"{working_dir}/test_losses.csv")
...
if __name__ == "__main__":
    ...
    if torch.cuda.is_available():
        ...
        mp.spawn(main, args=(world_size, args.total_epochs, args.save_every, args.data_dir, args.working_dir, args.batch_size, args.bayesian), nprocs=world_size)
    else:
        print(f"Cuda not available! Working with single threaded CPU.")
        main(rank=0, world_size=1, epochs=args.total_epochs, save_every=args.save_every, data_dir=args.data_dir, working_dir=args.working_dir, batch_size=args.batch_size, bayesian=args.bayesian)
